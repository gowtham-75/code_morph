...
BEGIN
    -- Validate order
    SELECT COUNT(*) INTO v_status FROM orders WHERE order_id = v_order_id;
    IF v_status = 0 THEN
        RAISE e_invalid_order;
    END IF;

    -- Fetch order and customer details
    SELECT customer_id, billing_address, shipping_address, order_priority INTO v_customer_id, v_billing_address, v_shipping_address, v_order_priority
    FROM orders
    WHERE order_id = v_order_id;
    ...
    FOR r_item IN c_order_items LOOP
        ...
        SELECT NVL(SUM(quantity), 0) INTO v_existing_quantity FROM order_items WHERE product_id = v_product_id AND order_id = v_order_id;

        IF v_stock_available + v_existing_quantity < v_quantity THEN
        ...
        UPDATE products SET stock_quantity = stock_quantity - (v_quantity - v_existing_quantity)
    ...
    UPDATE orders SET total_amount = v_total_amount, order_priority = v_order_priority, order_date = v_order_date WHERE order_id = v_order_id;

    -- Commit transaction
    COMMIT;

    -- Display success message
    DBMS_OUTPUT.PUT_LINE('Order modified successfully with Order ID: ' || v_order_id);
    DBMS_OUTPUT.PUT_LINE('Total Amount (after discount, tax, and shipping): ' || v_total_amount);

EXCEPTION
    WHEN e_out_of_stock THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Order modification failed: Insufficient stock for Product ID ' || v_product_id);
    WHEN e_invalid_order THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Order modification failed: Invalid order ID.');
    WHEN e_invalid_coupon THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Order modification failed: Invalid or expired coupon code.');
    WHEN e_invalid_shipping_address THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Order modification failed: Invalid shipping address.');
    WHEN e_insufficient_loyalty_points THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Order modification failed: Insufficient loyalty points.');
    WHEN OTHERS THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Order modification failed due to unexpected error: ' || SQLERRM);
END;
